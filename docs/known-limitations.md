...
- **The example cohort's track is synthetic, though its numbers are now this tool's.** The bundle
  ...
  `scripts/build-example-bundle.ts` from `tests/fixtures/synthetic-analysis.ts`, so the *track* is
  a scripted trajectory over the three real clips' frame counts and frame rates, not an animal. Its
  latencies, errors, paths and strategies are no longer scripted alongside it: since A2 every
  `derived` layer is recomputed by `derive()` from the bundle's own `auto ⊕ corrections` under its
  own parameters, and `tests/export/example-bundle-derived.test.ts` re-derives the shipped document
  to prove it. A real tracking run replaces `sourceSession()` with its outputs; the loader validates
  shape only and never reads a number, so the swap needs no code change. The *fingerprints* are real
  and measured, because the fetch button verifies a download against them (D33). The on-screen
  provenance line (`EXAMPLE_PROVENANCE_TEXT`) still says "illustrative, not a real tracking run",
  which remains true of the track and is due a rewording when the demo take lands.
...
- **The bundle is `.json.gz`, not plain `.json`.** 11.1 MiB of pretty-printed JSON for three
  ...
  to about 494 kB (505,918 B as committed; the exact figure moves whenever the bundle is

...
- **The export ZIP is stored, not compressed.** `src/export/zip.ts` writes store-only entries, so a
  bundle is as large as the files inside it: 10.8 MiB for the demo take's three-video cohort, of
  which 10.7 MiB is the session file carrying its embedded `auto` track layer. A twenty-video cohort
...
- **The example cohort is a real run, but it can only be rebuilt with the take's session file.**
  The bundle at `public/examples/example-cohort.barnestrack.json.gz` is the session saved from the
  recorded demo take at `bc81fe7` — a real tracking run of the three sample clips, with the
  corrections made during it — regenerated by `scripts/build-example-bundle.ts`. That script reads
  the source session from `BARNESTRACK_DEMO_SESSION`, because the file is 11 MB and the pre-commit
  guard refuses any staged blob over 2 MB, so it is not in the repo: on a machine without it the
  two tests in `tests/demo/bundle.test.ts` that re-run the builder skip (named for the variable),
  and everything asserted about the *committed* artifact still runs. Its `derived` layers are
  recomputed by `derive()` from the bundle's own `auto ⊕ corrections` under its own parameters
  (A2), and `tests/export/example-bundle-derived.test.ts` re-derives the shipped document to prove
  it. The *fingerprints* are real and measured, because the fetch button verifies a download
  against them (D33).
- **The bundle is `.json.gz`, not plain `.json`.** 10.7 MiB of pretty-printed JSON for three
  videos, and `.claude/hooks/pre-commit-guard.sh` refuses any staged blob over 2 MB; gzip brings it
  to about 585 kB (599,002 B as committed; the exact figure moves whenever the bundle is

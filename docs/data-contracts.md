...
| `strategy.serialMinRun`                | count              | 3       | Serial: a run of at least this many adjacent-hole investigations in one direction around the ring (a change of direction ends the run) with no centre crossing during it, before or ending at the target visit. | O7 |

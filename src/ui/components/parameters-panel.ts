 ...
 * and cannot be forgotten. Each row carries its unit, its default and the
 * definition sentence from `PARAMETER_DEFINITIONS` verbatim, so the meaning of
 * a threshold is one disclosure away from the control that changes it. This
 * project's own decision ids are not on screen: they are internal bookkeeping,
 * and a definition whose source is a paper names the paper in its own sentence.
 *
  ...
  function numberRow(path: ParameterPath): Row {
    ...
    const definition = disclosure('Definition', [
      ...
      el('p', {
        ...
        text: `Default ${formatParameterValue(parameterAt(DEFAULT_PARAMETERS, path))}`,
      }),
